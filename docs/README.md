...
| 11 | [从零构建大模型：预训练篇](./11-从零构建大模型：预训练篇.md) | 数据管线、Scaling Laws、AMP、AdamW、分布式并行 | ~35 min |
| 12 | [从零构建大模型：后训练与对齐篇](./12-从零构建大模型：后训练与对齐篇.md) | SFT、LoRA、RLHF/PPO、DPO/GRPO、蒸馏、安全对齐 | ~35 min |

**总计约 5.5 小时**：01–05 打地基（推理入门），06–10 建高楼（推理精通），11–12 追本溯源（训练视角：模型是怎么来的）。
